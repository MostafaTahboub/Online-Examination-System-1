...
const validateUserEnrollment = async (req: Request, res: Response, next: NextFunction) => {
...
const validateExamEnrollment = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!existingExam) {
        ...
        if (currentTime > existingExam.endTime) {
            ...
            return res.status(403).send(`Enrollment is not allowed becasuse the exam hasn't started yet, startTime: ${existingExam.startTime} `);
        }
        else {
            next();
        }
