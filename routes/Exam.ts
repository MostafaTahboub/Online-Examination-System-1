...
import { createExam, createExamRandom, shuffleArray, randomizeQuestions, updateExam, fetchQuestionsForExam } from '../controllers/exam.js';
...
router.post('/start', async (req, res) => {
    try {
        ...
        const currentTime = new Date();
        const examEndTime = new Date(exam.startTime.getTime() + exam.duration * 60 * 1000);
        console.log(exam.startTime);
        console.log(currentTime);

        if (currentTime < exam.startTime) {
            return res.status(400).send("You are a good student (NERD), but please wait until the exam starts.");
        }

        if (currentTime > examEndTime) {
            return res.status(400).send("The exam time has ended. See you in the summer.");
        }
        ...
        res.status(200).send("Exam started successfully. Be careful when submitting answers.");
    } catch (error) {
        console.error(error);
    ...
    try {
        ...
        if (decoded) {
            ...
            if (lastResponse) {
                ...
                if (currentExam) {
                    const currentTime = new Date();
                    const examEndTime = new Date(currentExam.startTime.getTime() + currentExam.duration * 60 * 1000);
                    ...
                    if (examEndTime < currentTime) {
                        ...
                        return res.status(404).send("The exam has finished. You can't submit. See you in the summer.");
                    }
                ...
                return res.status(500).send("Something went wrong. make the start exam before .");
            }
        } else {
            return res.status(400).send("Invalid token");
        }
        ...
        console.error(error);
        res.status(500).json({ message: "An error occurred while submitting the exam." });
    }
});
